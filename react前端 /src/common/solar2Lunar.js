...
// 阳历:以太阳周年视为运动周期,一周 365.2422为一年,一年划分为12个月,根据阳历日期,可知寒来暑往

// 阴历:以月亮的圆缺周期,为一月(29.53日),积累12个月为一年,因为阴历的日期不能显示四季冷暖,故基本废除

/*阴阳历: 我国的农历即为阴阳历,它取月相的变化周期及塑望月为月的长度,参考太阳的回归年为年的长度
 *通过设置闰月以使得平均年历与回归年相适应,更具农历的日期,可知道潮汐涨落,四季更替
*/

/*
月策与置末规则：朔日为每月起点（初一日）；朔日（月初一日）到下个朔日（下月初一日）有多少天、
这月就有多少天。有30天为大月、有29天为小月；唐朝武德二年（公元619年）以前用的是平朔，
武德二年（公元619年）以后用的是定朔。

年策与置闰规则：以冬至日必须在子月（寅正十一月），上个冬至月（寅正十一月）
到下个冬至月如有12个月就不置闰，如有13个月就要置闰，以上个冬至月之后第一个无中气的月份为闰月，
在几月后面就称闰几月。 
...
// 我国农历的一些原数据并没有一个固定的算法可以通过程序产生，只能由天文台测定后提供，
// 所以一般的所谓“万年历”都是采用“查表法”获取农历数据而产生的；这也就限定了“万年历”的区间范围。
// 源数据经过抓取以及匹配获得Js里查表法所需要用的“表”，


export default class LunarCalender {
    ...
    solar2lunar=(y, m, d) => { //参数区间1900.1.31~2100.12.31
        ...
        return {
            'day': this.toChinaDay(day) === "初一" ? ((isLeap ? "\u95f0" : '') + this.toChinaMonth(month)) : this.toChinaDay(day),
            'isTerm': isTerm, //
            'Term': Term,
        };
    }
